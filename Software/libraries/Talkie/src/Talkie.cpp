...
extern int effectMode;   // somewhere in your main .ino, define these
extern int effectLevel;

static int slurRepeatCounter = 0;
static int slurRepeatMax = 1;
static uint8_t savedFrameData[16];
static int16_t savedK[10];
static uint8_t savedEnergy = 0, savedPeriod = 0;

Talkie::GateCallbackFunc Talkie::gateCallback = nullptr;

void Talkie::setGateCallback(GateCallbackFunc cb) {
    gateCallback = cb;
}
...
if (gateCallback) gateCallback(true);  // to turn gate ON
...
#define PWM_OUTPUT_FUNCTION(nextPwm) analogWrite(13, nextPwm)
    analogWriteFrequency(13, 62500);
#  endif // (DAC_PIN)
...
if (gateCallback) gateCallback(false); // to turn gate OFF
 ...
 */
 // Add at file top if not already present:
 static int lastFrameRepeatCount = 0;
 static uint8_t lastSynthCoeffs[12];
...
static void setNextSynthesizerData() {
    ...
    // EffectMode 1
    // =================================================================================================================
    if (effectMode == 1 && effectLevel > 0 && slurRepeatCounter > 0) {
        // Repeat previous frame
        synthEnergy = savedEnergy;
        synthPeriod = savedPeriod;
        synthK1 = savedK[0];
        synthK2 = savedK[1];
        synthK3 = savedK[2];
        synthK4 = savedK[3];
    ...
    if (energy == 0) {
        ...
        synthEnergy = 0;
        synthK1 = 0; synthK2 = 0; synthK3 = 0; synthK4 = 0; synthK5 = 0;
        synthK6 = 0; synthK7 = 0; synthK8 = 0; synthK9 = 0; synthK10 = 0;
        savedEnergy = 0;
        savedPeriod = 0;
        memset(savedK, 0, sizeof(savedK));
        sPointerToTalkieForISR->setPtr(sPointerToTalkieForISR->FIFOPopFront());
        slurRepeatCounter = 0;
        slurRepeatMax = 1;
    } else {
        uint8_t repeat;
        uint8_t thisEnergy = pgm_read_byte(&tmsEnergy[energy]);
        repeat = sPointerToTalkieForISR->getBits(1);
        uint8_t thisPeriod = pgm_read_byte(&tmsPeriod[sPointerToTalkieForISR->getBits(6)]);
        int16_t thisK1, thisK2, thisK3, thisK4, thisK5, thisK6, thisK7, thisK8, thisK9, thisK10;

        if (!repeat) {
            thisK1 = (int16_t) pgm_read_word(&tmsK1[sPointerToTalkieForISR->getBits(5)]);
            thisK2 = (int16_t) pgm_read_word(&tmsK2[sPointerToTalkieForISR->getBits(5)]);
#if !defined(USE_10_BIT_KOEFFICIENT_VALUES)
            thisK3 = pgm_read_byte(&tmsK3[sPointerToTalkieForISR->getBits(4)]);
            thisK4 = pgm_read_byte(&tmsK4[sPointerToTalkieForISR->getBits(4)]);
            if (thisPeriod) {
                thisK5 = pgm_read_byte(&tmsK5[sPointerToTalkieForISR->getBits(4)]);
                thisK6 = pgm_read_byte(&tmsK6[sPointerToTalkieForISR->getBits(4)]);
                thisK7 = pgm_read_byte(&tmsK7[sPointerToTalkieForISR->getBits(4)]);
                thisK8 = pgm_read_byte(&tmsK8[sPointerToTalkieForISR->getBits(3)]);
                thisK9 = pgm_read_byte(&tmsK9[sPointerToTalkieForISR->getBits(3)]);
                thisK10 = pgm_read_byte(&tmsK10[sPointerToTalkieForISR->getBits(3)]);
            } else {
                thisK5 = thisK6 = thisK7 = thisK8 = thisK9 = thisK10 = 0;
            }
#else
            thisK3 = (int16_t) pgm_read_word(&tmsK3[sPointerToTalkieForISR->getBits(4)]);
            thisK4 = (int16_t) pgm_read_word(&tmsK4[sPointerToTalkieForISR->getBits(4)]);
            if (thisPeriod) {
                thisK5 = (int16_t) pgm_read_word(&tmsK5[sPointerToTalkieForISR->getBits(4)]);
                thisK6 = (int16_t) pgm_read_word(&tmsK6[sPointerToTalkieForISR->getBits(4)]);
                thisK7 = (int16_t) pgm_read_word(&tmsK7[sPointerToTalkieForISR->getBits(4)]);
                thisK8 = (int16_t) pgm_read_word(&tmsK8[sPointerToTalkieForISR->getBits(3)]);
                thisK9 = (int16_t) pgm_read_word(&tmsK9[sPointerToTalkieForISR->getBits(3)]);
                thisK10 = (int16_t) pgm_read_word(&tmsK10[sPointerToTalkieForISR->getBits(3)]);
            } else {
                thisK5 = thisK6 = thisK7 = thisK8 = thisK9 = thisK10 = 0;
            }
#endif
        } else {
            thisK1 = synthK1; thisK2 = synthK2; thisK3 = synthK3; thisK4 = synthK4; thisK5 = synthK5;
            thisK6 = synthK6; thisK7 = synthK7; thisK8 = synthK8; thisK9 = synthK9; thisK10 = synthK10;
        }

        synthEnergy = thisEnergy;
        synthPeriod = thisPeriod;
        synthK1 = thisK1;
        synthK2 = thisK2;
        synthK3 = thisK3;
        synthK4 = thisK4;
        ...
        // Save these values for repeating
        savedEnergy = thisEnergy;
        savedPeriod = thisPeriod;
        savedK[0] = thisK1;
        savedK[1] = thisK2;
        savedK[2] = thisK3;
        savedK[3] = thisK4;
        ...
        // Set how many repeats before advancing to next frame
        if (effectMode == 1 && effectLevel > 1) {
            slurRepeatMax = effectLevel - 1;
            slurRepeatCounter = slurRepeatMax;
        } else {
            slurRepeatCounter = 0;
            slurRepeatMax = 1;
        }
    ...
    // EffectMode 2
    // =================================================================================================================
    if (effectMode == 2 && sPointerToTalkieForISR->WordDataPointer != nullptr) {
        static uint8_t prevEnergy = 0, prevPeriod = 0;
        static int16_t prevK[10] = {0};
        static int glitchState = 0;      // 0 = normal, 1 = repeat, 2 = silence
        static int repeatCount = 0;

        // If repeating a frame, just re-output previous frame data N times
        if (glitchState == 1 && repeatCount > 0) {
            synthEnergy = prevEnergy;
            synthPeriod = prevPeriod;
            for (int i = 0; i < 10; i++) {
                (&synthK1)[i] = prevK[i];
            }
            repeatCount--;
        ...
        // If silencing a frame
        if (glitchState == 2 && repeatCount > 0) {
            synthEnergy = 0;
            synthPeriod = 0;
            for (int i = 0; i < 10; i++) {
                (&synthK1)[i] = 0;
        ...
        // Decide what glitch to apply (probabilities scale with effectLevel)
        int action = random(0, 100);
        int repeatProb = effectLevel * 2;      // 0..50%
        int silenceProb = effectLevel;         // 0..25%
        int skipProb = effectLevel;            // 0..25%

        if (action < repeatProb && effectLevel > 0) {
            // REPEAT last frame N times
            repeatCount = 1 + random(1, 1 + effectLevel / 3);
            glitchState = 1;
            return;
        } else if (action < repeatProb + silenceProb && effectLevel > 0) {
            // SILENCE N frames
            repeatCount = 1 + random(1, 1 + effectLevel / 2);
            glitchState = 2;
            return;
        } else if (action < repeatProb + silenceProb + skipProb && effectLevel > 0) {
            // SKIP: Fast-forward a whole frame (simulate dropped phoneme)
            for (int i = 0; i < 50; ++i) sPointerToTalkieForISR->getBits(1); // Approx 50 bits/frame
            glitchState = 0; // Next call resumes normal
            // Note: don't return here! Fall through and parse the next frame after skip
        } else {
            glitchState = 0;
        }

        // On a "normal" frame: parse as usual, but save it for repeat
        // (Do NOT return here, let the rest of setNextSynthesizerData() run)
    }

    // =================================================================================================================
    // EffectMode 3
    // =================================================================================================================
    if (effectMode == 3 && effectLevel > 0 && synthEnergy > 0) {
        // EXTREME vibrato, both speed and depth increase with effectLevel
        static int lfoPhase = 0;
        float minSpeed = 12.0f;   // slowest (lowest effectLevel)
        float maxSpeed = 1.2f;    // fastest (highest effectLevel)
        float speed = minSpeed - ((minSpeed - maxSpeed) * (effectLevel / 25.0f));
        // Clamp for safety
        if (speed < maxSpeed) speed = maxSpeed;

        float depth = 2.0f + effectLevel * 3.5f;   // much deeper with higher effectLevel
        float lfo = sin(2.0f * 3.14159f * (lfoPhase / speed));
        lfoPhase++;
        if (lfoPhase > 32000) lfoPhase = 0;

        int modPeriod = (int)synthPeriod + (int)(lfo * depth);
        if (modPeriod < 1) modPeriod = 1;
        if (modPeriod > 255) modPeriod = 255;
        synthPeriod = (uint8_t)modPeriod;

        // Metallic robot at higher levels
        if (effectLevel > 10) {
            int8_t kMod = (int8_t)(lfo * (effectLevel * 1.5f));
            synthK1 = synthK1 + kMod;
            synthK2 = synthK2 - kMod / 2;
        }
    }
    ...
    // EffectMode 4
    // =================================================================================================================
    if (effectMode == 4 && effectLevel > 0 && synthEnergy > 0) {
        static int lfoPhase = 0;
        float minSpeed = 8.0f;    // Lower minSpeed = more intense modulation
        float maxSpeed = 0.5f;
        float speed = minSpeed - ((minSpeed - maxSpeed) * (effectLevel / 25.0f));
        if (speed < maxSpeed) speed = maxSpeed;

        lfoPhase++;
        if (lfoPhase > 32767) lfoPhase = 0;

        // Use triangle wave for max “busy-ness” (less pure silence)
        float t = (lfoPhase / speed);
        float tri = 2.0f * fabs(fmod(t, 1.0f) - 0.5f); // triangle wave between 0..1
        float depth = 0.6f + 2.4f * (effectLevel / 25.0f); // 0.6-3.0 times

        // “Extremify”: Go all the way from ~10% to 300% energy
        float modulated = (synthEnergy * (0.1f + tri * depth));
        if (modulated > 255) modulated = 255;

        synthEnergy = (uint16_t)modulated;

        // Optional: sprinkle random spikes at high effect levels for extra dirt
        if (effectLevel > 20 && random(0, 12) == 0) {
            synthEnergy = 255;
        }
    }
